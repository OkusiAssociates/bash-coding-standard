...
type [-aftpP] name [name ...]
 ...
 type either returns the name of the disk file that would be executed if name were specified as a
 command name, or nothing if ``type -t name'' would not return file.  The -P option forces a PATH search
 ...
 functions, if and only if the -p option is not also used. The table of hashed commands is not
