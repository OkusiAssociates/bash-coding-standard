...
# JSON helper: Add validation result
add_json_result() {
  ...
  message=${message//\"/\\\"}  # Escape quotes

  json_entry="{\"check\":\"$check_name\",\"status\":\"$status\",\"message\":\"$message\"}"
...
# Run validation and record JSON result
run_validation_check() {
  ...
  if "$check_func"; then
    ((OUTPUT_JSON)) && add_json_result "$check_name" "pass" "Check passed"
  else
    result=$?
    ((OUTPUT_JSON)) && add_json_result "$check_name" "fail" "Check failed"
  fi

  # Don't propagate error - let all validations run
  return 0
}
...
main() {
  ...
  if ((OUTPUT_JSON)); then
    # JSON mode: suppress normal output, collect results
    QUIET=1
    VERBOSE=0
  fi
